...
%               'Still the second line!\r\nThe Third line!'];
    ...
    'while secs - startKbCheck < maxTime && ~any(keyCode(KbName(targetKey)))                    \r\n' ...    
        'if event.waitBar                                                                       \r\n' ...
...
questionStruct(4).name       = 'Wait bar';
questionStruct(4).sort       = 'checkbox';
questionStruct(4).data       = '';
questionStruct(4).tooltip    = 'if enabled, a bar will indicate how much time is left to respond';
   
questionStruct(5).name       = 'Location';
questionStruct(5).sort       = 'popupmenu';
questionStruct(5).data       = {'top','bottom'};
questionStruct(5).tooltip    = 'determines whether the waitbar will be shown at the top or bottom of the screen';
...
% it.
    ...
    elseif ismember(' ',waitFor.key)
        ...
        waitFor.key = cell(1);
        for i = 1:length(keys)
            waitFor.key{i} = keys(i);
        end        
    else
        waitFor.key = waitFor.key;
      end
